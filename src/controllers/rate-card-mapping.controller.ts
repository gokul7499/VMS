...
export const createRateCardMapping = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        reply.status(201).send({
            status_code: 201,
            message: 'Rate Card Mapping created successfully',
            rate_card_mapping: RateCardMappingData?.id,
            trace_id: traceId,
        });
    } catch (error) {
        reply.status(500).send({status_code:500, message: 'Error While Creating Rate Card Mapping', error, trace_id: traceId});
    }
};
...
export const updateRateCardMapping = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        if (data) {
            ...
            reply.status(200).send({status_code:200, message: 'Rate Card Mapping Data Not Found.' ,trace_id:traceId});
        }
    } catch (error) {
        reply.status(500).send({status_code:500, message: ' An Error Occurred While Updating The Rate Card Mapping.', error, trace_id: traceId });
    }
}
...
export const deleteRateCardMapping = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        if (!data) {
            return reply.status(200).send({status_code:200, message: 'Rate Card Mapping Data Not Found' ,trace_id:traceId});
        }
        ...
        reply.status(500).send({status_code:500, message: 'Error Deleting Rate Card Mapping Data', error, trace_id: traceId });
    }
}
...
export async function getAllRateCardMappings(
    ...
    try {
        ...
        reply.status(200).send({
            status_code: 200,
            message:" Rate Card Mapping Retrieved Successfully",
...
export async function getRateCardMappingById(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
        ...
        if (item) {
            ...
            reply.status(200).send({status_code:200, message: 'Rate Card Mapping Data Not Found', Rate_card_mapping: [] ,trace_id:traceId});
        }
    } catch (error) {
        reply.status(500).send({status_code:500, message: 'An Error Occurred While Fetching Rate Card Mapping Data.', error ,trace_id:traceId});
    }
}
