...
export const CandidateCodeGenerate = async (vendor_id: string, program_id: string): Promise<string> => {
    ...
    // Fetch latest candidate ID for the vendor
    ...
    if (lastCandidate && lastCandidate.candidate_id) {
        const match = lastCandidate.candidate_id.match(/-(\d+)$/);
        if (match) {
            nextNumber = (parseInt(match[1]) + 1).toString().padStart(5, '0');
        }
    }
